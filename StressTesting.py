...
class ScenarioAnalysis(model.MVPModel):
    ...
    def _get_portfolio_data(self):
        """Convert daily price data to monthly return
        Output: DataFrame of monthly return of each ticker """
    ...
    def _get_monthly_premium(self):
        """Get monthly return - risk free rate by ticker
        Output: DataFrame of monthly risk premium of each ticker """
    ...
    def _get_monthly_fama_factors(self):
        """Pull monthly fama french factors market/size/value
        Output: DataFrame of monthly fama french factors"""
    ...
    def _get_quarterly_fama_factors(self):
        """Pull quarterly fama french factors market/size/value
        Output: DataFrame of quarterly fama french factors"""
        df = pd.read_excel('Fama3.xlsx').set_index('Date')
        yr = [str(i)[:4] for i in df.index]
        m = [str(i)[4:] for i in df.index]
        l = [yr[i] + '-' + m[i] for i in range(len(yr))]
    ...
    def _get_scenario_data(self):
        """Pull assigned expected macro indicator wihtin three types of Brexit scenario
        Output: DataFrame of macro factors"""
        df = pd.read_excel('Brexit_scen.xlsx').set_index('Scenario')
        return df


    def _get_hist_macro(self):
        """Pull historical macro factors
        Output: DataFrame of quarterly macro factors"""
    ...
    def _get_macro_corr(self):
        """Pull macro factors' correlation matrix
        Output: DataFrame of correlation matrix of macro factors"""
        macro = self.hist_macro
        macro.corr().to_excel('Macro_factor_correlation.xlsx')
        return macro.corr()

    def _get_fama_model(self):
        """Regress historical returns on fama french factors
        Output: DataFrame of regression results including betas/alpha/rsquare/mse"""
    ...
    def _get_macrofama_model(self):
        """Rgress historical macro factors on fama french factors
        Output: DataFrame of regression results including betas and alphas"""
    ...
    def _get_fama_forecast(self):
        """Forecast expected fama french factors given scenario macro factors
         Output: Forecasted fama french factors within 3 different scenarios"""
    ...
    def _get_return_forecast(self):
        """Forecast expected return given fama factors
         Output: Forecasted monthly return within 3 different scenarios"""
    ...
    def _get_optimal_weight(self):
        """Forecast expected portfolio return given optimal weight of mvp
         Output: Forecasted annual portfolio total return within 3 different scenarios"""
